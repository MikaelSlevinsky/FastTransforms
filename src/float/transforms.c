...
static inline float Analphabeta(const int n, const float alpha, const float beta) {
    ...
    float t = fmin(fmin(fmin(alpha, beta), alpha + beta), 0.0);
...
float * plan_leg2cheb(const int normleg, const int normcheb, const int n) {
    ...
    float * lam = (float *) calloc(n, sizeof(float));
    float * sclrow = (float *) calloc(n, sizeof(float));
    float * sclcol = (float *) calloc(n, sizeof(float));
    for (int i = 0; i < n; i++) {
        lam[i] = lambda((float) i);
...
float * plan_cheb2leg(const int normcheb, const int normleg, const int n) {
    float * A = (float *) calloc(n * n, sizeof(float));
    float * lam = (float *) calloc(n, sizeof(float));
    float * lamh = (float *) calloc(n, sizeof(float));
    float * sclrow = (float *) calloc(n, sizeof(float));
    float * sclcol = (float *) calloc(n, sizeof(float));
    for (int i = 0; i < n; i++) {
        lam[i] = lambda((float) i);
...
float * plan_ultra2ultra(const int normultra1, const int normultra2, const int n, const float l1, const float l2) {
    float * A = (float *) calloc(n * n, sizeof(float));
    float * lam1 = (float *) calloc(n, sizeof(float));
    float * lam2 = (float *) calloc(n, sizeof(float));
    float * sclrow = (float *) calloc(n, sizeof(float));
    float * sclcol = (float *) calloc(n, sizeof(float));
    float scl = tgamma(l2)/tgamma(l1)/tgamma(l1-l2);
    for (int i = 0; i < n; i++) {
        lam1[i] = lambda2((float) i, l1 - l2, 1.0);
        lam2[i] = lambda2((float) i, l1, l2 + 1.0);
        sclrow[i] = normultra2 ? sqrt(2.0*M_PI*lambda2((float) i, 2.0*l2, 1.0)/(i+l2))/(pow(2.0, l2)*tgamma(l2)) : 1.0;
        sclcol[i] = normultra1 ? sqrt((i+l1)/(2.0*M_PI*lambda2((float) i, 2.0*l1, 1.0)))*(pow(2.0, l1)*tgamma(l1)) : 1.0;
    }
    for (int j = 0; j < n; j++)
        for (int i = j; i >= 0; i -= 2)
...
float * plan_jac2jac(const int normjac1, const int normjac2, const int n, const float alpha, const float beta, const float gamma) {
    float * A = (float *) calloc(n * n, sizeof(float));
    float * lam1 = (float *) calloc(n, sizeof(float));
    float * lam2 = (float *) calloc(2*n, sizeof(float));
    float * sclrow = (float *) calloc(n, sizeof(float));
    float * sclcol = (float *) calloc(n, sizeof(float));
    for (int i = 0; i < n; i++) {
        lam1[i] = lambda2((float) i, alpha - gamma, 1.0);
        sclrow[i] = (2.0*i + gamma + beta + 1.0)*lambda2((float) i, gamma + beta + 1.0, beta + 1.0);
        sclcol[i] = lambda2((float) i, beta + 1.0, alpha + beta + 1.0)/tgamma(alpha - gamma);
    }
    for (int i = 0; i < 2*n; i++)
        lam2[i] = lambda2((float) i, alpha + beta + 1.0, gamma + beta + 2.0);
    if (beta + gamma == -1.0)
