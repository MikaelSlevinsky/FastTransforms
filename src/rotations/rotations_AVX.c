#include "rotations.h"
    ...
    static inline void apply_givens_AVXc(const double S, const double C, ft_complex * X, ft_complex * Y) {
        apply_givens_AVX(S, C, (double *) X, (double *) Y);
    }
    static inline void apply_givens_t_AVXc(const double S, const double C, ft_complex * X, ft_complex * Y) {
        apply_givens_t_AVX(S, C, (double *) X, (double *) Y);
    }
    static inline void apply_twisted_givens_AVXc(const double S, const double C, ft_complex * X, ft_complex * Y) {
        double * XD = (double *) X;
        double * YD = (double *) Y;
        double4 x = vloadu4(XD);
        double4 y = vloadu4(YD);
        vstoreu4(XD, vas4(C*x,  S*y));
        vstoreu4(YD, vas4(C*y, -S*x));
    }
    static inline void apply_twisted_givens_t_AVXc(const double S, const double C, ft_complex * X, ft_complex * Y) {
        double * XD = (double *) X;
        double * YD = (double *) Y;
        double4 x = vloadu4(XD);
        double4 y = vloadu4(YD);
        ...
        if (s >= 0)
            for (int j = MIN(am, as)-1; j >= 0; j--)
                for (int k = n-2-abs(am-as)-j; k >= 0; k--)
                    apply_twisted_givens_AVXc(SRP->s2(k, j, abs(am-as)), SRP->c2(k, j, abs(am-as)), A+k*S, A+(k+1)*S);
        else
            for (int j = MIN(am, as)-1; j >= 0; j--)
                for (int k = n-2-abs(am-as)-j; k >= 0; k--)
                    apply_twisted_givens_t_AVXc(SRP->s2(k, j, abs(am-as)), SRP->c2(k, j, abs(am-as)), A+k*S, A+(k+1)*S);
        for (int j = abs(am-as)-2; j >= (am+as)%2; j -= 2)
            for (int k = n-3-j; k >= 0; k--)
                apply_givens_AVXc(SRP->s1(k, j), SRP->c1(k, j), A+k*S, A+(k+2)*S);
    }
    void kernel_spinsph_lo2hi_AVX(const ft_spin_rotation_plan * SRP, const int m, ft_complex * A, const int S) {
        ...
        for (int j = (am+as)%2; j <= abs(am-as)-2; j += 2)
            for (int k = 0; k <= n-3-j; k++)
                apply_givens_t_AVXc(SRP->s1(k, j), SRP->c1(k, j), A+k*S, A+(k+2)*S);
        if (s >= 0)
