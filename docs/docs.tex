...
\begin{equation}
f_n^s(\theta,\varphi) = \sum_{\ell=0}^{n}\sum_{m=-\ell}^{+\ell} f_\ell^m Y_{\ell,m}^s(\theta,\varphi).
\end{equation}
If spherical harmonic expansion coefficients are organized into the array:
\begin{equation}
F = \begin{pmatrix}
...
That is:
\begin{equation}
g_{2n}(r,\theta) = \sum_{\ell=0}^{n}\sum_{m=-2n}^{+2n} g_{2\ell}^m \frac{e^{\ii m\theta}}{\sqrt{2\pi}} \left\{\begin{array}{lr} \cos[2\ell\cos^{-1}(r)] & m~{\rm even},\\ \cos[(2\ell+1)\cos^{-1}(r)] & m~{\rm odd}.\end{array}\right.
\end{equation}
